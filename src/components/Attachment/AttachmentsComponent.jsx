...
export default class AttachmentsComponent extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    ...
    newProps.viewAttachments ? this.setState({ attachments: newProps.viewAttachments }) : null
  }
  ...
  handleFileChange() {
    ...
    if (savedTarget.files) {
      ...
      if (allowedMimeTypes.includes(receivedFile.type) && receivedFile.size <= allowedSize) {
        ...
        delete attachments[type]
        stateObj['loading' + type] = false
        stateObj['responseError' + type] = { arabicMessage: <FormattedMessage id='attachmentType' />, message: <FormattedMessage id='attachmentType' /> }
        stateObj['uploadClass' + type] = 'failedUpload'
        this.setState(stateObj)
      } else if (receivedFile.size > allowedSize) {
        delete attachments[type]
        stateObj['loading' + type] = false
        stateObj['responseError' + type] = { arabicMessage: <FormattedMessage id='attachmentSize' />, message: <FormattedMessage id='attachmentSize' /> }
        stateObj['uploadClass' + type] = 'failedUpload'
        this.setState(stateObj)
      }
    } else {
    ...
    return (
      <div>
        <InlineMessageComponent
          message={<FormattedMessage id='infoMessage0002'values={{ types, fileSize: attachmentConfig[0].maxSize / 1024 / 1024 }} />}
          type='info'
        />
        <div className='masonry'>
          {attachmentConfig.map(attachment =>
            (isAuthElm == '2' && attachment.attachmentTypeEn == 'Ownership' ? null :
              ...
              {((!specific && attachment.attachmentConfigID !== conditionOn) || (!specific && !isNotAvailable)) || attachment.attachmentConfigID === specific ?
                  ...
                  className={classNames('item', this.state['uploadClass' + attachment.attachmentConfigID] ? this.state['uploadClass' + attachment.attachmentConfigID] : ' ', this.state.className)}
                >
                  ...
                  {Object.values(attachments)
                      ...
                      <a
                            onClick={this.handleViewImage.bind(this, this.props.viewAttachments ?
                          '/api/v1/ftp/' + Object.values(attachments)
                            .find(file => file.attachmentSystemFileName.includes(attachment.attachmentTypeEn)).requestID +
                              '/' + attachment.attachmentConfigID
                          : '/api/v1/ftp/tmp/' + Object.values(attachments)
                          ...
                          >
                            <img
                          width='90%'
                          src={this.props.viewAttachments ? '/api/v1/ftp/' + Object.values(attachments).find(file => file.attachmentSystemFileName.includes(attachment.attachmentTypeEn)).requestID + '/' + attachment.attachmentConfigID : '/api/v1/ftp/tmp/'
                      ...
                      : Object.values(attachments)
                        .find(file => file.attachmentSystemFileName.includes(attachment.attachmentTypeEn) &&
                        file.attachmentExt === 'pdf') ?
                        <a href={this.props.viewAttachments ?
                          '/api/v1/ftp/' + Object.values(attachments)
                            .find(file => file.attachmentSystemFileName.includes(attachment.attachmentTypeEn)).requestID
                            + '/' + attachment.attachmentConfigID
                          : '/api/v1/ftp/tmp/'
                        + Object.values(attachments)
                          ...
                          + '/' + attachment.attachmentTypeEn}
                          >
                            <i className='fa fa-file-pdf-o attachmentIcon' aria-hidden='true' />
                          </a> : null}
                  ...
                  {window.location.href.indexOf('querySubscriptionRequest') != -1 || window.location.href.indexOf('viewSubscriptionRequest') != -1 ? null :
                    <label className='label' id={attachment.attachmentConfigID} htmlFor={attachment.attachmentConfigID + 'File'}>
                        ...
                        <FormattedMessage id='dragNdrop' />
                          : null}
                        <br />{this.state['fileName' + attachment.attachmentConfigID]}
                      </label>}
                  {window.location.href.indexOf('querySubscriptionRequest') != -1 || window.location.href.indexOf('viewSubscriptionRequest') != -1 ? null :
                      this.state['uploadClass' + attachment.attachmentConfigID] === 'successUpload' ?
                        <ActionButtonComponent type='Delete' onClick={e => this.deleteAttachment(e, attachment.attachmentConfigID)} />
                        : null}
                  {window.location.href.indexOf('querySubscriptionRequest') != -1 || window.location.href.indexOf('viewSubscriptionRequest') != -1 ? null :
                      attachment.required ?
                        this.state.requiredAttachmentError ?
                          <InlineMessageComponent
                            message={<FormattedMessage id='errorMessage.isRequired' values={{ fieldName: this.props.lang === AR ? attachment.attachmentTypeAr : attachment.attachmentTypeEn }} />}
                            type='uploadError'
                          /> : null
                        : null}
        ...
        <RecaptchaComponent callAPI={this.renderCaptcha} lang={this.props.lang} />
      </div>
    )
  }
}
